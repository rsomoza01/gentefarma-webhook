...
function parseSelectionCommand(text) {
  ...
  const hasDosageOrForm = /\b(\d+(?:[.,]\d+)?\s*(mgr|mg|mcg|g|gr|ml|cc|ui|iu)|tab|tabs|tabl|tabletas?|capsulas?|capsules?|cap|caps|ampollas?|suspension|susp|jarabe|gotas|crema|gel|polvo|polvos|unguento|sobres?|retad(?:ar|or)?|retard(?:ar|ado|ada)?)\b/.test(normalized);
...
function extractMedicineRequests(text) {
  ...
  // DIAGNOSTIC: log every call to trace TABL/MGR/X15 spurious entries
  console.log('🩺 [EXTRACT-MED] INPUT="%s" normalized="%s"', String(text).slice(0,80), normalized.slice(0,80));
  ...
  console.log('🩺 [EXTRACT-MED] OUTPUT=%s results=%d', JSON.stringify(results), results.length);
...
if (/^(dr\.?|dra\.?|doctor|doctora|medico|médico)\b/i.test(raw)) return '';
  ...
  const isDoseToken = (token) => /^(\d+(?:[.,]\d+)?|mg|mcg|g|gr|ml|cc|ui|iu|mL|mgr|tab|tabs|tabl|tabletas?|capsulas?|capsules?|cap|caps|ampollas?|suspension|susp|jarabe|gotas|crema|gel|polvo|polvos|unguento|unguentos|sobres?|retad(?:ar|or)?|retard(?:ar|ado|ada)?)$/i.test(token);
...
function extractMedicineQuery(text) {
  ...
  const unitList = 'mgr|mg|mcg|g|gr|ml|cc|ui|iu|mL|tab|tabs|tabl|tabletas?|capsulas?|capsules?|cap|caps|ampollas?|suspension|susp|jarabe|gotas|crema|gel|polvo|polvos|unguento|unguentos|sobres?|retad(?:ar|or)?|retard(?:ar|ado|ada)?';
...
const MED_FORM_TOKENS=new Set([
    ...
    'cápsula', 'cápsulas', 'cap', 'caps', 'suspension', 'suspensión', 'susp', 'jarabe', 'gotas', 'crema', 'gel', 'polvo', 'polvos',
    'unguento', 'unguentos', 'sobres', 'sobresa', 'retad', 'retadar', 'retardar', 'retardado', 'retardada', 'capsules', 'tablet', 'tabletass',
    'tab', 'tabs', 'tabl',  // tablet abbreviations (OCR variations)
    'mgr', 'mgrs',          // miligramos abbreviation
    // Pharmaceutical salt forms — never standalone medicines
    'potasico', 'potásico', 'sodico', 'sódico', 'clorhidrato', 'maleato', 'besilato', 'sulfato', 'nitrato', 'fosfato', 'acetato', 'diclorhidrato', 'bromuro'
  ]);
